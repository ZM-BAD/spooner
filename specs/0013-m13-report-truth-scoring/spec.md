...
- **Per-check quality matrix** (deterministic signals):
  ...
  - **cfg-lint / cfg-format** (0.5 each): config and command 0.4; + CI job 0.5; config or command alone 0.2. Formatter configs include `ruff.toml` (ruff provides lint + format — same file counts in both checks, symmetry 2026-08-07); lint configs include php (phpcs.xml / phpstan.neon / psalm.xml), format configs include `.php-cs-fixer` (2026-08-07); the evidence never claims a CI step when no CI file exists (report truth)
