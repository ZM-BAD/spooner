...
- **Existing spec directories are respected**: if the repo already has `specs/` or `.agents/notes/`, stage 4 installs no `docs/sdd/` templates, no sdd workflow, and appends no SDD convention to AGENTS.md — the report says "stage 4 skipped: existing spec dir … — respecting it"
