...
| `.github/workflows/ai-native.yml` (new file, never edits existing CI; **absent in no-workflow mode** — non-GitHub CI platform, spec 0008) | `templates/ci-workflow.yml.tpl` (warn-only lint/test/security jobs + declared-commands hard gate + a commit-msg commitlint check on the last commit) |
