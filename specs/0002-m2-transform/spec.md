...
| `.pre-commit-config.yaml` (if absent) | `templates/pre-commit-config.yaml.tpl` (markdownlint + gitleaks + hygiene hooks + **local hooks**: declared-only `typecheck` + `test`, scoped `stages: [pre-commit]` — pre-commit becomes a superset of CI, so a green pre-commit implies a green CI; repos without those npm scripts skip them; the stages scope keeps them out of the commit-msg stage, where the commitlint step runs in CI without node_modules) |
| `.markdownlint-cli2.yaml` (if absent) | `templates/markdownlint-cli2.yaml.tpl` |
| `.github/workflows/ai-native.yml` (new file, never edits existing CI; **absent in no-workflow mode** — non-GitHub CI platform, spec 0008) | `templates/ci-workflow.yml.tpl` (warn-only lint/test/security jobs + declared-commands hard gate + a commit-msg commitlint check on the last commit; the pre-commit job runs with `SKIP: typecheck,test` — the local hooks need node_modules the job does not install, and both are covered elsewhere: typecheck in the declared-commands hard gate, test in lint-test) |
