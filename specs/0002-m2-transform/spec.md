...
|---|---|
| `.commitlintrc.json` (if absent — skipped when the repo has its own commitlint config: `.commitlintrc*` / `commitlint.config.*` / package.json `commitlint` field, with a notice; an installed `.commitlintrc.json` would shadow the repo's config via cosmiconfig resolution order, dogfood review 2026-08-10) | `templates/commitlintrc.json.tpl` |
