...
- **Build verification (stage 2 rule)**: a missing tool (exit 127, "command not found") is reported as **tool missing, not a failing build** — the stage-2 message says "mvn is not installed (exit 127: command not found) — install the tool and re-run; a missing local tool is not a failing build"
