...
- **Same M8 treatment for hook tools as for CI platforms**: the probe gains the hook-tool question; a repo whose owner keeps husky/lefthook (or an existing hook tool) gets a skip with an explicit notice — never a foreign gate file. **Kept-hook-tool repos still get the commitlint discipline delivered**: the stage-2 report points at `templates/lefthook-commit-msg.yml` (an integration template — the commit-msg job the user merges into their own lefthook.yml) plus the manual `lefthook install` step (SKILL.md documents it). The template is a documented hand-off, never written over the user's lefthook.yml (same conflict red line as AGENTS.md).
...
- **GitHub reachability (three-layer answer)**: the generated config's hook repos are fetched from GitHub when pre-commit runs — with GitHub unreachable (no mirror/proxy), pre-commit cannot prepare the hook environment and commits are **blocked**. The generated header documents this; it is the tool's architecture, not a config bug — CI (GitHub Actions) is unaffected. Three mechanisms make the gate usable in intranet/air-gapped environments:
  ...
  3. **`--offline` variant**: generates a `repo: local`-only config — `repo: meta` checks (zero deps), self-contained perl implementations of trailing-whitespace / end-of-file-fixer (byte-aligned with pre-commit-hooks behavior, pinned by tests), `language: system` entries for pre-commit-hooks / gitleaks when the binaries are already installed; npm-managed hooks (markdownlint/commitlint/eslint/ruff) are **honestly omitted with install guidance** (declared-only philosophy — never fake coverage). The variant's header states it is the offline form and how to switch back.
...
- **TOOL_VERSION bump** (new template bytes + behavior change): **0.14.0** + baked `EXPECTED` sync in the workflow templates + a local version-ledger row (spec 0004/0005 contract).
...
- **Automatic lefthook/husky config generation** — a kept lefthook repo gets the `templates/lefthook-commit-msg.yml` integration template + manual steps, never a generated/merged lefthook.yml (a YAML merge into the user's hook config is a foreign-file class; husky generation stays demand-driven — WXT-repo evidence documented, not shipped)
...
14. **Regression + contract**: typecheck + markdownlint + full suite green; TOOL_VERSION bumped with EXPECTED sync + local version-ledger row; stack workflow templates' SKIP lists extended per stack (python: pytest,pip-audit / go: gofmt,go-vet,go-test / java: java-test); **self-apply**: stage-2 dry-run on this repo reports its own config as `conflict` (the repo-specific manifest-consistency hook is user-owned — the red line working as designed), while the legacy-upgrade path and determinism are covered by dedicated fixtures
...
- **Version churn** — TOOL_VERSION 0.14.0 with EXPECTED sync + local version-ledger row (spec 0004/0005 contract)
