...
14. **Regression + contract**: typecheck + markdownlint + full suite green; TOOL_VERSION bumped with EXPECTED sync + docs/08 ledger row; stack workflow templates' SKIP lists extended per stack (python: pytest,pip-audit / go: gofmt,go-vet,go-test / java: mvn-test); **dogfood**: stage-2 dry-run on this repo reports its own config as `conflict` (the repo-specific manifest-consistency hook is user-owned — the red line working as designed), while the legacy-upgrade path and determinism are covered by dedicated fixtures
