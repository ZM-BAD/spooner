...
function checkCfgLint(root: string): CheckResult {
  ...
  const config =
    hasPattern(root, [
      ...
      /^phpstan\.neon/,
      /^psalm\.xml/,
    ]) ?? ktlintConfigOf(root) ?? (ruff?.lint ? ruff.label : null);
...
function generatedRuffGate(root: string): { lint: boolean; format: boolean; label: string; formatLabel: string } | null {
...
function checkCfgFormat(root: string): CheckResult {
  ...
  const config =
    hasPattern(root, [
      ...
      /^ruff\.toml/,
      /^\.php-cs-fixer/,
    ]) ?? ktlintConfigOf(root) ?? gate?.config ?? (ruff?.format ? ruff.formatLabel : null);
  const cmd =
