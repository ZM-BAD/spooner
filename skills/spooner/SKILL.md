...
- **GitHub reachability for pre-commit** — the generated `.pre-commit-config.yaml` fetches its hook repos (pre-commit-hooks, markdownlint-cli2, commitlint, gitleaks) from GitHub when pre-commit runs. With GitHub unreachable (no mirror/proxy configured), pre-commit cannot prepare the hook environment and commits are **blocked** — the generated config's header says so. CI (GitHub Actions) is unaffected. **Intranet/air-gapped environments** (spec 0010): stage 2 probes GitHub and prints mirror guidance when unreachable; `--hook-mirror <base>` rewrites the managed repo URLs to an intranet mirror (plus the optional `git config --global url."<base>/".insteadOf "https://github.com/"` — git's insteadOf IS honored by pre-commit, but only at global/system level, and npm/PyPI/GOPROXY registries need their own mirrors); `--offline` generates a repo:local-only config that never fetches GitHub (hygiene checks are self-contained, npm-managed hooks are honestly omitted with a header note). `--offline` and `--hook-mirror` are mutually exclusive. The probe is read-only with a 10s timeout — the first stage-2 run on a slow network can wait up to 10s before the mirror guidance appears.
...
node <skill-dir>/scripts/audit.ts --root /path/to/repo [--format json|markdown] [--verify] [--verify-command <cmd>]
node <skill-dir>/scripts/transform.ts --root /path/to/repo [--stage 2|3|4|all] [--dry-run] [--ci github|gitlab|none] [--gates warn-only|hard] [--verify-timeout <minutes>] [--verify-command <cmd>] [--hook-mirror <base>] [--offline] [--format json|markdown]
...
- `audit.ts` defaults to JSON (machine-parseable); `--format markdown` prints the human-readable report. Command tracing is **static** — `agents-commands` evidence marks "static trace, not executed"; `--verify` actually runs the traced lifecycle commands (build/test, same command family as transform stage 2 — `--verify-command <cmd>` overrides it, a pnpm/lerna/rush workspace degrades to the lightest declared command) and the evidence reports passed / FAILED (exit + stderr) / tool not installed (exit 127 anywhere in the stderr — a missing local tool is not a failing build).
- `transform.ts` defaults to `--stage all` status; `--stage 2|3|4` applies one stage, `--dry-run` shows the plan without writing anything. Build verification reports **three states** — green / failing (real failure + reason) / unverifiable (timeout or missing tooling): a timeout says "verification could not complete" and suggests a lighter `--verify-command` (never "already failing"); failures carry environment diagnostics (node_modules / npm ls) and the heartbeat labels each command's position in the verification family (a `--verify-command` composite additionally names its current `&&`/`;` sub-step).
...
6. **Verify it took effect — not just that files were written** (the config ≠ enforcement lesson, productized): after each applied stage, prove the gates are real.
   ...
   - Run `pre-commit run --all-files` once — the hooks execute; nothing is silently dead. **First verify without the auto-fix hygiene hooks**: `SKIP=trailing-whitespace,end-of-file-fixer pre-commit run --all-files` (spec 0015 — on a large legacy repo the auto-fix hooks rewrite hundreds to thousands of files per run: 358 / 209 / 119 / 114 / 1961 files observed, each reverted as noise). Then run the two hygiene hooks once (`pre-commit run trailing-whitespace end-of-file-fixer --all-files`) — their rewrites are the formatter's output: commit them if this is the repo's own gate run, revert them as noise if you are only verifying on an upstream copy. Make sure the worktree is restorable (`git stash`, a clean clone, or `git checkout -- <paths>`) before any full run.
   - **A first run that fails on pre-existing repo debt is the gate working, not an install failure** — hard gates are local ⊇ CI by design, so legacy repo docs (markdownlint violations — MD040/MD012/MD014 are the common ones) block the first full run. **Triage by scale**: a handful of violations (≤ ~20) → fix them; a large debt (real-world corpora run to thousands — a 24-file Go monorepo corpus produced ~21k errors across MD022/MD032/MD024) → fixing is not realistic in one pass — run with `SKIP=markdownlint-cli2` and report it as a finding, or extend the generated `.markdownlint-cli2.yaml` `ignores` list for vendored/upstream docs. Either way the owner decides; never roll back the install (a Makefile-less Go repo's 5 violations = fixable, a Go monorepo's ~21k = SKIP — same gate, different triage).
...
| 4 — SDD (optional)                               | spec/plan/tasks templates + SDD convention in AGENTS.md + spec-existence CI gate (**platform-routed like stage 2** — the gate installs only where the GitHub workflow applies; skipped with "… (SDD spec gate)" otherwise); **existing spec docs are respected** — if the repo already has `specs/` or `.agents/notes/` with markdown specs, stage 4 skips `docs/sdd/` + the SDD append entirely and says so                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 | `docs/sdd/*.md`, `.github/workflows/sdd.yml` (absent in no-workflow mode), AGENTS.md convention                                                                                                               |
...
| ruby / swift / dotnet / harmonyos                                                        | ✅     | ✅ (under-scores only, never over)                                                                                  | ⚠️ cross-stack gates only + explicit "not supported yet" notice                     | —                                                       |
| apple (Xcode/iOS/macOS; Tuist / CocoaPods / Carthage)                                    | ✅     | ✅ (xcodebuild build/test lifecycle credit)                                                                         | ⚠️ cross-stack gates only + explicit "not supported yet" notice                     | —                                                       |
| c/cpp (CMake / Meson / vcpkg / Conan)                                                    | ✅     | ✅ (cmake --build / ctest lifecycle credit)                                                                         | ⚠️ cross-stack gates only + explicit "not supported yet" notice                     | —                                                       |
| dart/flutter (pubspec.yaml — merged stack, static match)                                 | ✅     | ✅ (flutter test test-only + dart analyze lint credit)                                                              | ⚠️ cross-stack gates only + explicit "not supported yet" notice                     | —                                                       |
| unity (ProjectVersion.txt + Assets/)                                                     | ✅     | ✅ (under-scores only — no canonical lifecycle)                                                                     | ⚠️ cross-stack gates only + explicit "not supported yet" notice                     | —                                                       |
| zig                                                                                      | ✅     | ✅ (zig build/test lifecycle credit)                                                                                | ⚠️ cross-stack gates only + explicit "not supported yet" notice                     | —                                                       |

**Primary stack rule**: node > python > go > java > rust for mixed repos (one workflow per repo). Non-supported stacks get an explicit notice — never silent npm gates. The audit's `agents-commands` check credits the stack lifecycle commands too (go/rust/java repos can now score 2/2); php test signals (phpunit.xml / phpunit in composer.json) are traced beyond the primary stack in mixed node+php repos. Spec 0014 extended detection to 15 stacks — Tier-1 (apple / c-cpp / dart-flutter / unity) and zig stay detect+audit only (the rest of the Tier-2 batch is deferred until the skill has real adoption); subdir-only signals (React Native's ios/android trees) do not inflate detection.
...
- Root: . · Dry-run: true · Templates: installed 0.9.0 → current 0.12.0

| File                    | Stage | Status     | Version        |
| ----------------------- | ----- | ---------- | -------------- |
| .commitlintrc.json      | 2     | outdated   | 0.9.0 → 0.12.0 |
...
- sync never overwrites a `modified` file (user edits at the current template version) — it replaces only `outdated`/`missing` template files and reports the rollback command (`git restore …`); `outdated` = older recorded version + byte diff, so user edits on an OLD-version install are replaced too (spec 0004 risk register) — dry-run first
