...
> Read this file first, then `README.md` and `specs/` for the live work contract. Local-only notes (`HANDOFF.md`) are not published.
...
- Product design frozen; milestone history lives in git commits, never here
- Engineering scaffold ready: TypeScript 6 zero-build, SDD workflow (`specs/`), pre-commit + markdownlint + commitlint, GitHub Actions
- All specs shipped (0001-0006, 0008-0016): the audit → transform → check → sync loop is complete — the installed CI workflow hard-gates manifest consistency (drift → red) and the generated pre-commit config hard-gates the ledger locally (self-contained manifest-consistency hook, baked EXPECTED); transform supports node / python / go / java (Maven + Gradle incl. kotlin/Android `build.gradle.kts`/`settings.gradle(.kts)`) / rust (stack-aware lifecycle + per-stack CI workflows + generated stack-aware pre-commit gates — prettier gets a local `--write` hook running the project's own prettier when declared (the sole auto-fix exemption: deterministic formatter; the managed mirror lags prettier releases), and SKILL.md authors get a local skills-ref validation hook (missing tool → skip with a notice) — the local gate set covers CI's checks; **intranet/air-gapped support: stage 2 probes GitHub, `--hook-mirror <base>` rewrites managed repo URLs, `--offline` emits a repo:local-only config**; husky/lefthook/yorkie hook ecosystems skip the config with a notice — a dead husky dependency without hooks config does NOT skip; non-GitHub CI platforms skip the workflow with a notice; gate strictness is a Stage-0 question landing as `--gates warn-only\|hard` — hard renders the workflow's quality jobs without `continue-on-error` (template bytes stay warn-only; recorded in the manifest)); the installed commitlint gate enforces (install step + CI commit-msg check + gate-active audit); the readiness badge matches the README's dominant badge style (5 shields styles, fixed `AI-Readiness` label); the audit scores 0-10 on deterministic quality signals with two-sourced fix hints and a monorepo sub-stack note — PHP signals score too (composer.lock / phpunit / php-cs-fixer) even though transform's supported stacks stop at rust; stack detection covers 15 stacks (spec 0014: apple / c-cpp / dart-flutter / unity + zig — detect+audit only, canonical lifecycle credit where unambiguous; Tier-2 rest deferred); next candidates: launch prep
...
| `npm run test:coverage`                         | node:test suite + lcov coverage (writes `coverage/lcov.info`, CI uploads to Codecov)                                                                                                                                                                                                                                                                                                                         |
...
| `node skills/spooner/scripts/audit.ts`          | AI-Readiness scoring (/10, M1; optional `--root <path>` / `--format markdown` / `--verify` executes the traced lifecycle commands instead of marking them unverified / `--verify-command <cmd>` overrides the verified command)                                                                                                                                                                              |
| `node skills/spooner/scripts/check.ts`          | drift check (M3: baseline delta + manifest drift; optional `--root <path>` / `--format markdown`)                                                                                                                                                                                                                                                                                                            |
| `node skills/spooner/scripts/transform.ts`      | transform workflow (M2: stages 2-4 + manifest consistency; optional `--root <path>` / `--stage 2/3/4/all` / `--dry-run` / `--ci github\|gitlab\|none` / `--gates warn-only\|hard` / `--verify-timeout <minutes>` / `--verify-command <cmd>` / `--hook-mirror <base>` / `--offline` / `--format markdown`; build verification is three-state + monorepo-aware; stage 2 only uses `--offline`/`--hook-mirror`) |
...
| `node skills/spooner/scripts/badge.ts`          | readiness badge (M9: 5 shields styles + README style probe + fixed `AI-Readiness` label; optional `--root <path>` / `--style <name>` / `--format markdown`)                                                                                                                                                                                                                                                  |
...
3. Changing frozen design: review the local decision log first, then update `HANDOFF.md` (local)

## Development playbook

**The loop**: change code → `npm run check` (typecheck + lint:md + tests) → `pre-commit run --all-files` (16 hooks, incl. typecheck + tests — a green pre-commit implies a green CI) → push the branch (every branch push runs both workflows) → user approves merge/push to main.

**Do**

- **Contract docs are current-state only, never history**: specs (a fix updates the spec **in place** — no acceptance logs, no fix history, no version transitions) and the AGENTS.md/SKILL.md status sections (current capability set only — no milestone bullets, no dates, no "shipped" language). History lives in commit messages + `HANDOFF.md` (local) — never in the contract docs
...
- **A pitfall that repeats must produce a prevention mechanism, not another patch** (spec 0015, the 19-round repeating-pitfall review): record + fix is not enough — every dogfood round ends with either a regression test that would have caught the class (stack-parity / pinned-byte assertions) or a fixed process step (e.g. `SKIP=trailing-whitespace,end-of-file-fixer` in gate verification), or the round records the rejection reason. Repeating classes — fresh-deps lag ×5, audit/artifact contradiction ×3, template-literal backslash ×2, auto-fix side effects ×5 — are exactly what this rule exists to retire
- **Template change ⇒ bump `TOOL_VERSION`** + baked `EXPECTED` in all five workflow templates + a local version-ledger row (spec 0004/0005 contract) — then dogfood `sync`; **mirror content changes (not just EXPECTED/SKIP) to the installed dogfood workflow in the same commit** — `--stage 2` refuses via conflict (the M12 python3 switch missed the installed workflow; the review caught it)
- **A feature branch ships ONE version over main** — intermediate per-batch TOOL_VERSION bumps fold into the branch's single version (main 0.10.0 → branch 0.11.0); per-batch bump semantics hold only on main (each merged batch carries its own bump)
- Test version assertions import `TOOL_VERSION` dynamically — never hard-code it (a bump will silently rot the tests)
- **Squash by substance, not by default**: before squashing, read what each commit actually changed (`git show --stat <sha>` / `git diff <sha>^ <sha>` — the commit messages alone are not enough) and group them by real function: a spec's slices, a dogfood round's fixes, a review batch. Squash to the number of natural units (2-4 is typical); **never default to a single commit** — a one-commit branch hides the review trail, makes rollback coarse, and erases per-function history (2026-08-10: a 13-commit branch was squashed to 1 by default; the single-commit exception applies only when the branch IS one unit of work)
...
- `npm run check` + `pre-commit run --all-files` (16 hooks) before any commit
- CI simulation: `SKIP=typecheck,test,eslint,manifest-consistency pre-commit run --all-files` → 12 Passed + 4 Skipped
...
- **`language: system` entries never run through a shell** — pre-commit shlex-splits the entry and execs the first token directly: shell builtins (`command`) fail resolution ("Executable `command` not found", Linux CI 2026-08-10) and `$(...)`/`|`/`;` become literal args. macOS's `/usr/bin/command` masks builtin usage locally as a false green (the hook does nothing and still "Passes") — wrap shell logic in `bash -c '...'` (prettier pattern; generator guard: transform.test.ts sweep over local-hook entries)
...
- **package-lock.json's top-level `version` is npm metadata snapshotted at lock creation — manual package.json bumps leave it behind silently** (the lockfile sat at 0.1.0 while package.json reached 0.12.0; no gate could see it — npm ci only checks the lock's dependency tree, never the top-level version). Prevention: the dogfood `.pre-commit-config.yaml` carries a local version-sync gate (python3, check-only, baked at the tail of the config) failing a commit on a split with the `npm version <v> --no-git-tag-version` fix named — dogfood-only, deliberately NOT in the generator (2026-08-15)
...
- **Test fixture names must not case-collide with system files** (`claude.md` aliases `CLAUDE.md` on macOS's case-insensitive APFS, not on Linux ext4): a stage-3 symlink test that wrote its target as `claude.md` passed green on macOS while the real behavior was platform-divergent — Linux CI would have gone red. When a test's behavior depends on a path that looks like a known file, use a non-colliding name (e.g. `contract.md`) so macOS can't mask the truth (2026-08-10, review round 2 HIGH-1; "green pre-commit implies green CI" only holds when local can't fake it)
...
- **Version ledger**: dependency upgrades go through the local version ledger (not published) — update it and mention the upgrade in the commit; no opportunistic upgrades
- Repo docs are English-first; local handoff notes are Chinese and local-only
