...
**The loop**: change code → `npm run check` (typecheck + lint:md + tests) → `pre-commit run --all-files` (15 hooks, incl. typecheck + tests — a green pre-commit implies a green CI) → push the branch (every branch push runs both workflows) → user approves merge/push to main.
...
- `npm run check` + `pre-commit run --all-files` (15 hooks) before any commit
- CI simulation: `SKIP=typecheck,test,eslint,manifest-consistency pre-commit run --all-files` → 11 Passed + 4 Skipped
