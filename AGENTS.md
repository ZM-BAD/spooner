...
| `node skills/spooner/scripts/transform.ts`      | transform workflow (M2: stages 2-4 + manifest consistency; optional `--root <path>` / `--stage 2/3/4/all` / `--dry-run` / `--ci github\|gitlab\|none` / `--gates warn-only\|hard` / `--format markdown`) |
